...
const formacion = [
  ...
  { year: '2017', title: 'Terapia Vibracional y Sonido', inst: 'IACT · International', type: 'Certificación' },
  { year: '2019', title: 'Flores de Bach', inst: 'BFRP · Dr. Edward Bach Foundation', type: 'Certificación' },
...
export default async function ElTerapeutaPage() {
  return (
    <div style={{ background: 'var(--color-canvas)' }}>
      ...
      {/* ── HERO ── */}
      <section style={{ background: 'var(--color-canvas)', padding: '72px 48px 96px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
          <div>

            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 52, fontWeight: 500, color: 'var(--color-ink)', lineHeight: 1.1, marginBottom: 8 }}>
            ...
            <div style={{ background: 'var(--color-surface-soft)', borderRadius: 'var(--rounded-lg)', padding: '20px 24px', marginBottom: 28, borderLeft: `4px solid var(--color-primary)` }}>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 15, fontWeight: 400, lineHeight: 1.75, color: 'var(--color-ink)' }}>
                <strong>BioReset360 no es una clínica ni un equipo de psicólogos.</strong> Es la plataforma que la Dra. Patricia Rozo diseñó integrando ciencia clínica con herramientas de optimización del bienestar. Cada sesión, cada plan, cada decisión clínica pasa directamente por su rigor y criterio.
              </p>
            </div>

            <p style={{ fontFamily: 'var(--font-body)', fontSize: 16, fontWeight: 400, lineHeight: 1.9, color: 'var(--color-body)', marginBottom: 36 }}>
              Especializada en la intersección entre neurociencia clínica y psicología contemplativa, con más de 12 años acompañando personas en procesos de transformación profunda. Fundadora y única directora clínica del Método Enfoque 360.
            </p>
            <div style={{ display: 'flex', gap: 24, marginBottom: 40 }}>
              {[['12+', 'Años de experiencia'], ['300+', 'Pacientes acompañados'], ['6', 'Especializaciones']].map(([n, l]) => (
                <div key={l}>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: 30, fontWeight: 500, color: 'var(--color-ink)', lineHeight: 1 }}>{n}</div>
          ...
          <div style={{ position: 'relative' }}>
            <div className="hover-card" style={{ borderRadius: 'var(--rounded-lg)', height: 520, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: 32, position: 'relative', overflow: 'hidden', boxShadow: '0 24px 60px rgba(0,0,0,0.1)' }}>
              <Image
              ...
              />
              <div style={{ borderRadius: 'var(--rounded-md)', background: 'rgba(255,255,255,.08)', backdropFilter: 'blur(8px)', padding: '18px 20px', border: '1px solid rgba(255,255,255,.12)' }}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 400, color: '#ffffff', marginBottom: 4 }}>{"\"Cada consulta es un acto de escucha profunda.\""}</div>
                <div style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'rgba(255,255,255,.7)' }}>Dra. Patricia Rozo</div>
              </div>
            </div>
            <div className="hover-card-subtle" style={{ position: 'absolute', top: 28, right: -20, background: '#fff', borderRadius: 'var(--rounded-md)', padding: '14px 18px', boxShadow: '0 8px 32px rgba(0,0,0,.1)', border: '1px solid var(--color-hairline)', zIndex: 10 }}>
      ...
      {/* ── FORMACIÓN + MÉTODO ── */}
      <section style={{ background: 'var(--color-canvas)', padding: '96px 48px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80 }}>
            <div>
              ...
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 38, fontWeight: 500, color: 'var(--color-ink)', lineHeight: 1.15, marginBottom: 40 }}>Una trayectoria construida con rigor y ciencia</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {formacion.map((it, i) => (
                  <div key={i} style={{ display: 'flex', gap: 24, paddingBottom: 24, marginBottom: 24, borderBottom: i < formacion.length - 1 ? '1px solid var(--color-hairline)' : 'none' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                        ...
                        <div style={{ fontFamily: 'var(--font-body)', fontSize: 10.5, fontWeight: 500, color: 'var(--color-primary)', background: 'var(--color-surface-soft)', padding: '2px 8px', borderRadius: 999, flexShrink: 0, marginLeft: 8 }}>{it.type}</div>
                      </div>
            ...
            <div>
              ...
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 38, fontWeight: 500, color: 'var(--color-ink)', lineHeight: 1.15, marginBottom: 24 }}>Donde la ciencia y la experiencia convergen</h2>
      ...
      {/* ── CTA ── */}
      <section style={{ background: 'var(--color-surface-soft)', padding: '80px 48px' }}>
        <div style={{ maxWidth: 700, margin: '0 auto', textAlign: 'center' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 42, fontWeight: 500, color: 'var(--color-ink)', lineHeight: 1.15, marginBottom: 20 }}>Tu camino hacia la optimización empieza aquí.</h2>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 16, fontWeight: 400, color: 'var(--color-body)', lineHeight: 1.8, marginBottom: 40 }}>Si tienes dudas sobre el método o quieres saber si BioReset360® es para ti, contáctame directamente.</p>
          <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="https://wa.me/573000000000" target="_blank" rel="noreferrer" style={{ fontFamily: 'var(--font-body)', fontSize: 16, fontWeight: 500, color: '#fff', padding: '14px 32px', borderRadius: 'var(--rounded-lg)', background: '#25D366', display: 'inline-flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
