...
// Precios y entregables provienen del modelo real (data/planes.ts). headline/sub son el marco emocional del triaje.
const RESULTS = {
  essencial: { plan: 'Essencial', col: 'var(--color-signature-coral)', bg: 'var(--color-surface-soft)', price: '730.000', headline: 'Tu momento pide atención y cuidado.', sub: 'Comenzar es el acto más valiente. El Plan Essencial es una intervención focal de cuatro semanas con la Dra. Rozo — claridad clínica y herramientas concretas para tu situación.', perks: ['4 sesiones individuales de 60 min', 'Herramientas de trabajo intersesión', 'Nota de cierre del terapeuta', 'Orientación de continuidad'] },
  vital: { plan: 'Vital', col: 'var(--color-signature-forest)', bg: 'var(--color-surface-soft)', price: '1.471.000', headline: 'Estás listo/a para un proceso más profundo.', sub: 'El Plan Vital integra ocho semanas de acompañamiento psicoterapéutico con enfoque, dirección y entregables concretos para construir tu propósito.', perks: ['8 sesiones individuales de 60 min', 'Documento Tu Propósito Vital® sellado', 'Mapa de identidad personal', 'Acompañamiento directo entre sesiones'] },
  quantum: { plan: 'Quantum', col: 'var(--color-signature-yellow)', bg: 'var(--color-surface-soft)', price: '2.229.000', headline: 'Buscas transformación integral. Es tu momento.', sub: 'El Plan Quantum es el nivel más profundo: doce semanas de reestructuración integral que trabajan la sombra, el cuerpo y el legado.', perks: ['12 sesiones individuales de 60 min', 'Propósito Quantum® sellado', 'Plan de vida — 12 meses', 'Mensajería prioritaria 12 semanas'] },
};
...
export default function TriajePage() {
    ...
    setTimeout(() => {
      transition(() => {
        ...
        if (newAnswers.length === total) {
          // Breve pausa de "análisis" antes de mostrar el resultado
          setAnalyzing(true);
          setStep(total + 1);
  ...
  const planKey = sum <= 7 ? 'essencial' : sum <= 11 ? 'vital' : 'quantum';
  ...
  return (
    <div style={{ background: 'var(--color-canvas)', minHeight: '100vh' }}>
      ...
      <div>
          ...
          <div style={{ minHeight: '80vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(16px)', transition: 'opacity .35s ease,transform .35s ease' }}>
            <div style={{ maxWidth: 620, width: '100%' }}>
              <FadeIn>
                ...
                <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(26px, 6vw, 48px)', fontWeight: 500, color: 'var(--color-ink)', lineHeight: 1.2, textAlign: 'center', marginBottom: 'clamp(32px, 6vw, 56px)', letterSpacing: '-0.5px' }}>{QUESTIONS[step - 1].q}</h2>
              </FadeIn>
        ...
        {/* Analyzing — pausa breve antes del resultado */}
        {step > total && analyzing && (
          <div style={{ minHeight: '80vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', textAlign: 'center' }}>
            <FadeIn>
              <div style={{ width: 72, height: 72, borderRadius: 24, background: 'var(--color-surface-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 32px' }}>
                <svg width="34" height="34" viewBox="0 0 32 32" fill="none" aria-hidden="true"><path d="M16 4 Q22 10 22 18 C22 22.4 19.3 25 16 25 C12.7 25 10 22.4 10 18 Q10 10 16 4Z" stroke="var(--color-primary)" strokeWidth="1.5" fill="none" /></svg>
              </div>
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(24px, 5vw, 36px)', fontWeight: 500, color: 'var(--color-ink)', lineHeight: 1.25, marginBottom: 12 }}>
                Preparando tu recomendación…
              </h2>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 15, color: 'var(--color-muted)' }}>
                Estamos leyendo tus respuestas con cuidado.
              </p>
            </FadeIn>
          </div>
        )}
            ...
            <div style={{ minHeight: '80vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 24px 60px', textAlign: 'center' }}>
              <FadeIn>
                <div style={{ maxWidth: 640, width: '100%' }}>
                  ...
                  <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(30px, 6.5vw, 52px)', fontWeight: 500, color: 'var(--color-ink)', lineHeight: 1.15, marginBottom: 20, letterSpacing: '-1px' }}>{r.headline}</h2>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(16px, 3vw, 19px)', fontWeight: 400, lineHeight: 1.8, color: 'var(--color-body)', marginBottom: 44 }}>{r.sub}</p>
                  ...
                  {/* Otros ritmos — la recomendación no es una jaula */}
                  <div style={{ marginTop: 28, paddingTop: 24, borderTop: '1px solid var(--color-hairline)' }}>
                    <div style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--color-muted)', marginBottom: 12 }}>¿Prefieres otro ritmo?</div>
                    <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
                      {(Object.keys(RESULTS) as Array<keyof typeof RESULTS>).filter(k => k !== planKey).map(k => (
                        <Link key={k} href={`/pago?plan=${k}`} style={{
                          ...
                          Plan {RESULTS[k].plan} · ${RESULTS[k].price}
                        </Link>
                      ))}
                    </div>
                    <div style={{ marginTop: 16 }}>
                      <Link href="/precios" style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--color-muted)', textDecoration: 'underline', textUnderlineOffset: 3 }}>Comparar los tres planes</Link>
                    </div>
                  </div>

                  <div style={{ marginTop: 28 }}>
                    <button onClick={() => { setStep(0); setAnswers([]); setSelected(null); setEmail(''); setEmailSent(false); }} style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--color-muted)', background: 'none', border: 'none', cursor: 'pointer' }}>Repetir cuestionario</button>
                  </div>
                </div>
              </FadeIn>
            </div>
